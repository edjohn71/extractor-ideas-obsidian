...
const s: Record<string, React.CSSProperties> = {
  ...
  title:   { fontFamily: "var(--font-brand)", fontSize: 26, fontWeight: 700, color: "var(--text)", lineHeight: 1.25, marginBottom: 6 },
  sub:     { fontSize: 13, color: "var(--muted)", lineHeight: 1.5 },
  ...
  btn:     { width: "100%", padding: "14px", background: "var(--accent)", color: "#000", border: "none", borderRadius: "var(--radius)", fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 15, cursor: "pointer", letterSpacing: 0.5 },
  ...
  progress: { background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "var(--radius-lg)", padding: "20px", marginBottom: 12 },
...
export default function Home() {
  ...
  const procesar = async () => {
    ...
    setEstado("procesando"); setError(""); setImagenesExtras([]);
    try {
      const r = await fetch("/api/ejecutar-skill", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tipo, input }) });
  ...
  const reiniciar = () => {
    setEstado("idle"); setInput(""); setFileName(""); setTipo("youtube");
    setFramesActivos([]); setFramesEnNota([]); setFramesDisponibles([]);
    setImagenesExtras([]); setError("");
  };
      ...
      <div style={s.wrap}>
        ...
        {(estado === "idle" || estado === "procesando") && (
          <div style={s.card}>
            ...
            {estado === "idle"
              ? <button style={input.trim() ? s.btn : s.btnDis} onClick={procesar} disabled={!input.trim()}>Analizar →</button>
              : (
                <div style={s.progress}>
                  <div style={{ fontSize: 13, color: "var(--muted2)", marginBottom: 14 }}>Claude está analizando. <strong style={{ color: "var(--text)" }}>No cierres esta página.</strong></div>
                  <div style={s.dots}>
                    {[0,0.2,0.4,0.6,0.8].map((d,i) => <div key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--accent)", animation: `pulse 1.4s infinite ${d}s` }} />)}
                    <span style={{ fontSize: 12, color: "var(--muted)", marginLeft: 8 }}>Procesando...</span>
                  </div>
                </div>
              )
            }
